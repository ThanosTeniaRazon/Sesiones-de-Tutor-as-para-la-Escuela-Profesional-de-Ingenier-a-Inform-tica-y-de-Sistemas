...
namespace SysTutoria
{
     class Cdocente
    {
        ...
        private void conectar()
        {
            ...
            con.ConnectionString = "Server=DESKTOP-V5UUCMT\\SQLSERVVER; Initial Catalog=tutoria;Integrated Security = True;";
            try
            {
                con.Open();
            }
        ...
        public DataTable buscardocente()
        {
            this.conectar();//invoco a lA CONEXION
            DataTable Tabla = new DataTable();//declaro un data table(contenedor de datos)
            //para obtener los dATOS DE LA BASE DE DATOS
            string B = "select CodigoDocentes, Nombre + ' ' + Paterno + ' ' + Materno as datos from Docentes ";

            SqlDataAdapter da = new SqlDataAdapter(B, this.con);
            da.Fill(Tabla);//llevamos los datos
            return Tabla;//le damos un valor a uncion
        }
        public DataTable buscar_datos_docente()
        {
            this.conectar();//invoco a lA CONEXION
            DataTable Tabla = new DataTable();//declaro un data table(contenedor de datos)
            //para obtener los dATOS DE LA BASE DE DATOS
            string B = "select CodigoDocentes,Nombre + ' ' + Paterno + ' ' + Materno as datos FROM Docentes WHERE CodigoDocentes = '" + this.CodigoDocentes + "'";
        ...
        public string ObtenerDatosUsuario()
        {
            ...
            string consulta = "SELECT Nombre + ' ' + Paterno + ' ' + Materno as datos FROM Docentes WHERE CodigoDocentes = '" + this.CodigoDocentes + "'";
            SqlCommand cmd = new SqlCommand(consulta, this.con);

            using (SqlDataReader lector = cmd.ExecuteReader())
            {
                if (lector.Read())
                {
                    datosUsuario = lector["datos"].ToString();
                }
            }
